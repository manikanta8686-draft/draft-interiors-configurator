...
import { getSofaModelById, legFinishes, resolveSofaModel } from "../configurator/configuration";
...
import SalesSpecificationPanel from "./SalesSpecificationPanel.jsx";
...
} from "../sales/configurationSummary.js";
...
function ConfiguratorView() {
  ...
  const configurationKey = [design.modelId, design.fabricId, design.colourId, design.size, design.legs, design.cushions].join(":");
  const configurationReference = useMemo(() => createConfigurationReference({
    createId: () => `${globalThis.crypto.randomUUID()}-${configurationKey}`,
  }), [configurationKey]);
  const salesQuote = useMemo(() => buildSalesConfiguration({
    configuration: design,
    model,
    selectedFabric,
    selectedColour,
  ...
  }), [configurationReference, design, model, pricing, selectedColour, selectedFabric]);

  return <main className="config-page">
    <section className="config-heading"><p className="eyebrow">DRAFT / MADE TO ORDER</p><h1>{model.name}, <em>your way.</em></h1><p>Refine every material and detail. Your estimate updates as you design.</p></section>
    <div className="config-layout">
      ...
      <div className="viewer-panel">
        <div className="viewer"><div className="viewer-caption"><span>Interactive room preview</span></div><ErrorBoundary fallback={(retry) => <div className="viewer-fallback" role="status"><strong>Preview temporarily unavailable</strong><span>Your configuration and estimate remain available.</span><button type="button" onClick={retry}>Try preview again</button></div>}><SofaViewer {...viewerConfiguration} /></ErrorBoundary></div>
        <div className="viewer-note"><span>Drag to rotate - pinch to zoom</span></div>
        <div className="summary">
          <div><p>YOUR DESIGN</p><h2>{model.name}</h2><span>{selectedFabric.name} - {selectedColour.name} - {design.size}</span><small>{design.legs} legs - {design.cushions} cushions</small></div>
          <div><p>ESTIMATED TOTAL</p><motion.strong aria-live="polite" key={pricing.total} initial={reducedMotion ? false : { opacity: 0.55, y: 2 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: reducedMotion ? 0 : PREMIUM_MOTION.uiDuration, ease: PREMIUM_EASE }}>{formatINR(pricing.total)}</motion.strong><dl className="price-breakdown"><div><dt>{pricing.base.label}</dt><dd>{formatINR(pricing.base.amount)}</dd></div>{appliedAdjustments.map((item) => <div key={item.id}><dt>{item.label}</dt><dd>{formatINR(item.amount, { showSign: true })}</dd></div>)}</dl><small>Estimated delivery: 6-8 weeks</small></div>
          <button type="button" className="add-cart" onClick={() => document.getElementById("configuration-name")?.focus()}>{saved ? "Design saved" : "Save design"}<span>+</span></button>
          <a className="whatsapp" href="#personal-quote">Request a quote</a>
        </div>
      </div>
    </div>
    <SalesSpecificationPanel quote={salesQuote} />
    <SavedConfigurations quote={salesQuote} />
    <section className="quote-enquiry" id="personal-quote">
      <div><p className="eyebrow">PERSONAL QUOTE</p><h2>Talk through this design.</h2><p>Send configuration {configurationReference} to our design team. Every selected specification is included, and the estimate will be recalculated securely before we respond.</p><SalesSpecificationPanel quote={salesQuote} compact /></div>
      <EnquiryForm source="configurator" configuration={design} defaultMessage={buildEnquiryMessage(salesQuote)} submitLabel="Request my quote" createWhatsAppUrl={(customerName) => createWhatsAppQuoteUrl({ customerName, reference: configurationReference, model: model.name, fabric: selectedFabric.name, colour: selectedColour.name, size: design.size, legs: design.legs, cushions: design.cushions, estimatedTotal: formatINR(pricing.total) })} />
    </section>
  </main>;
}
